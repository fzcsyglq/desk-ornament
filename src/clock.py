...
class Frame(tk.Frame):
    ...
    def show_clock(self):
        ...
        res = requests.get(url,
                           proxies={"http": "http://10.40.58.203:10809",
                                    "https": "https://10.40.58.203:10809"
                           })
        print(url)
        if res.ok:
            data = res.json()
        ...
        def update_forecast():
            ...
            for image in idImage_array:
                self.wn.addshape(image)

        self.canvas = tk.Canvas(self, width = 400, height = 240)
        self.wn = turtle.TurtleScreen(self.canvas)
        self.wn.bgcolor("black")
        self.wn.screensize()
        #wn.setup(width=600, height=600)
        # Make full screen
        #self.wn.setup(width=1.0, height=1.0)
        #self.wn.title(wn_title)
        ...
        #self.canvas.pack(in_ = self)
        #self.top_window = self.canvas.winfo_toplevel()
